...
Claude Code can spawn subagents through tool events named `Agent` or `Task`,
depending on the Claude Code build. That is useful for parallel work, but it can
also consume a lot of usage quickly or become noisy when too many agents are
started at once.
...
- Hold budget-blocked subagent attempts until the user extends the budget.
...
- Gate new subagents at 75% of the 5-hour window and hard-stop them at 85% by default.
...
The hard gate is scoped to `PreToolUse` for the subagent tool (`Agent` or `Task`).
That is the only place where normal default enforcement denies work.
...
- Queued and budget-blocked subagent count.
- Verified subagent token total.
- Total recorded duration.
- 5-hour usage, warning gate, and hard ceiling when statusLine data is available.
- Per-subagent status, type, description, model, tokens, duration, and tool-call count.
- A note that token totals are verified after completion, not mid-run.
- Thinking/effort only when Claude Code exposes it to hooks.
